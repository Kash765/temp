...
def crash_out_move(init_face_detections, servo, picam2, face_detector, expression_handler):
    if(not init_face_detections or len(init_face_detections["faces"]) == 0):
        return
    
    expression_handler.angry()
    ...
    for angle in range(-90, 91, 5): # rotate 5 degrees per step
    ...
    try:
        while True:
            ...
            if not working and not on_break:
                crash_out_move(face_detections, servo, picam2, face_detector, expression_handler)
            else:
                expression_handler.sleepy()
